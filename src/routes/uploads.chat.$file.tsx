...
async function handleChatUpload(params: UploadParams, request: Request, head = false) {
    ...
    try {
        const buf = await readFile(
            join(process.cwd(), 'uploads', 'chat', params.file),
        );
...
export const Route = createFileRoute('/uploads/chat/$file')({
    server: {
        handlers: {
            GET: async ({ params, request }) => handleChatUpload(params, request),
            HEAD: async ({ params, request }) => handleChatUpload(params, request, true),
        },
    },
});
