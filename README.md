...
3. **Переменные окружения** (в панели, вкладка «Настройки»):
   ...
   - `WEB_ALLOWED_HOSTS` — ваш домен (например `movienest.example.ru`)
...
**Ограничение:** файловая система контейнера эфемерна — загруженные пользователями постеры (`uploads/`) пропадут при редеплое. Для продакшена стоит перенести хранение постеров в S3 (у Timeweb есть S3-хранилище).
