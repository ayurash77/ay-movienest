...
Веб-библиотека фильмов, сериалов и мультфильмов: карточки, группировки по происхождению и странам, страницы с подробной информацией. Зарегистрированные пользователи могут добавлять записи и выставлять оценки (1–5 звёзд).
...
src/
  routes/            file-based роуты
    index.tsx        Фильмотека: все карточки с группировками и фильтрами
    movies/index     категории «Фильмы» / «Сериалы» / «Мультфильмы»: поиск + сортировка
  ...
  server/            server functions и серверная логика
    ...
    movie-lookup.ts  быстрое заполнение формы через Wikipedia/Wikidata без AI-токенов
...
- По умолчанию карточки группируются как «Отечественные» / «Зарубежные», затем по странам. Пользователь может отключить любую группировку и скрыть отдельные страны.
- Разделы «Фильмы», «Сериалы», «Мультфильмы» фильтруют записи по `Movie.kind`; старая база мигрирует в `MOVIE`.
- Оценка пользователя — upsert: повторная оценка заменяет старую.
